...
def run_query(query,database=DATABASE_NAME):
    ...
    estimated_cost=(bytes_scanned / (1024**4)) * 5
    print(f"  Data scanned: {mb_scanned:.3f} MB  (~${estimated_cost:.8f})")
    
    result_key=f'athena-query-results/{query_id}.csv'
    result=s3.get_object(Bucket=BUCKET_NAME,Key=result_key)
    df=pd.read_csv(BytesIO(result['Body'].read()))
    return df
